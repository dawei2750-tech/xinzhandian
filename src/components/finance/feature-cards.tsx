...
export function FeatureCards() {
  return <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{featureCards.map((card) => <article key={card.title} className="panel panel-hover relative flex min-h-52 overflow-hidden p-5"><div className="relative z-10 flex min-w-0 flex-1 flex-col"><h2 className="font-semibold">{card.title}</h2><p className="mt-1 text-xs text-muted">{card.subtitle}</p><div className="mt-5"><p className="text-xs text-muted">{card.valueLabel}</p><p className={`mt-1 text-xl font-medium ${card.tone === "orange" ? "text-warning" : "text-ink"}`}>{card.value}</p></div><button className="primary-button mt-auto w-fit rounded-control px-4 py-2 text-sm font-medium">{card.button}</button></div><GlowIcon name={card.icon} tone={card.tone} label={card.title} size="xl" className="absolute -bottom-2 -right-2 rotate-6 opacity-90"/></article>)}</section>;
}
