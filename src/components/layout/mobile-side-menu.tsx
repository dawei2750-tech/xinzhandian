...
export function MobileSideMenu() {
  ...
  const drawer = <div data-testid="mobile-drawer" className="fixed inset-0 z-[80] lg:hidden">
    <button aria-label={text.common.closeMenu} onClick={() => setOpen(false)} className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
    <aside className="relative flex h-full w-[82%] max-w-sm flex-col overflow-y-auto border-r border-line-bright bg-canvas p-5 shadow-2xl">
      <div className="flex items-center justify-between"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-control bg-brand-gradient"><Icon name="chain" className="size-6" /></span><strong className="text-xl">{brand.name}</strong></div><button aria-label={text.common.closeMenu} onClick={() => setOpen(false)} className="grid size-10 place-items-center rounded-full border border-line text-xl text-muted">×</button></div>
      <nav aria-label={text.common.mobileMenu} className="mt-10 grid gap-3">
        {mobileDrawerItems.slice(0, 4).map((item, index) => <a key={item.href} href={item.href} onClick={() => setOpen(false)} className={`flex items-center gap-4 rounded-panel border p-4 ${index === 0 ? "border-warning/60 bg-warning/10 text-warning" : "border-line bg-surface-soft text-ink"}`}><Icon name={item.icon} label={labels[index]} className="size-6" /><span className="text-lg font-medium">{labels[index]}</span><span aria-hidden="true" className="ml-auto text-muted">›</span></a>)}
